...
const ResultSearch = () => {
    ...
    const [valueSearchHome, setValueSearchHome, roomChatUser, setRoomChatUser, userLogin, setUserLogin, filterSearch, filterMessage] = useContext(ResultSearchUserContext)

    const userId = JSON.parse(localStorage.getItem('userId'))
    const id = userId && userId._id

    const { imageUrl, name } = userLogin && { ...userLogin }
                                ...
                                {filterSearch && filterSearch.length > 0 ?
                                    filterSearch.map((e, i) => {
                                        return (
                                            <CardUser
                                                key={i}
                                                img={`${e.imageUrlUser1 === imageUrl ? e.imageUrlUser2 : e.imageUrlUser1}`}
                                                nameCard={e.nameUser1 === name ? e.nameUser2 : e.nameUser1}
                                            // clickCard={() => {
